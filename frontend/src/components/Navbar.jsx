...
const Navbar = ({ login, setLogin, size }) => {
    ...
    return (
        <>
           ...
           <div className="flex lg:justify-around justify-between relative  items-center py-5 lg:px-10 px-5 bg-white shadow-md">
                <Link to={"/"}><h1 className="text-3xl font-bold">Shop <span className="text-purple-500">Sizzle</span> </h1></Link>
                <ul className="gap-10 font-semibold hidden lg:flex">
                    <li className="hover:cursor-pointer hover:text-gray-600"><NavLink to={"/home"}>Home</NavLink></li>
                ...
                <div className="flex gap-8 text-gray-800">
                    ...
                    <div className="relative hover:-translate-y-1 transition-all">
                        ...
                        <span className="absolute -right-2 -top-2 bg-blue-400 rounded-[50%] w-5 h-1 text-center text-sm pb-5">{size}</span>
                    </div>    
                    </Link>
                    {
                        login ? 
                ...
                <svg xmlns="http://www.w3.org/2000/svg" onClick={handleNavbar} className="block lg:hidden cursor-pointer" width="24" height="24" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 6h18M3 12h18M3 18h18" /></svg>
                 ...
                 <div className=" w-64 top-0 h-[100vh] absolute z-50 right-0 -mr-72 bg-white transition-all duration-700" id="navbar">
